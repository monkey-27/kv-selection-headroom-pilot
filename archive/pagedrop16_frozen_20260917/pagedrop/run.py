"""Single-wakeup, example-batched, atomic/resumable full MATH500 pilot."""
import hashlib,json,time,sys
from pathlib import Path
import torch
from .cache import PilotCache,patch_qwen,POLICIES
from kvheadroom.runtime import GPUHourBudget

ROOT=Path('/outputs/pagedrop16_v1')
SEED=20260915
...
def run(commit):
    ...
    data_path=Path('/outputs/data/math/test.jsonl'); synth_path=Path('/outputs/kv_headroom_v1/synthetic_examples.json')
    ...
    spec={'version':'pagedrop16-v1','model':'Qwen/Qwen3-4B','model_revision':'1cfa9a7208912126459214e8b04321603b3df60c','seed':SEED,'example_rng':'seed + sha256(prompt_ids) first32bits + layer_idx; per-example generators independent of batching/resume','policies':POLICIES,
          ...
          'cap_h100_hours':4,'backend':'flashinfer native page16 + SDPA causal prefill'}
    ...
    budget=GPUHourBudget(ROOT,3.85,1.0,0.03)
    ...
    with budget.active('single_worker_including_load_validation_eval_microbench'):
        ...
        report=analyze(ROOT); atomic(ROOT/'report.json',report); commit()
        if report['accuracy_passes']:
            from .bench import bench
            atomic(ROOT/'microbenchmark.json',bench(model.config,budget)); budget.flush('microbenchmark'); commit()
        print(json.dumps(report),flush=True)
    commit()
    return {'run_dir':str(ROOT),'report':json.loads((ROOT/'report.json').read_text()),'hours':budget.used_hours}
